...
for i=1:m
    y_m(i,y(i,1)) = 1;
    J = J + 1/m * ( (-y_m(i,:) * log(h(i,:))' ) - ( (1 - y_m(i,:)) * log(1 - h(i,:))' ));
end

% My attempt at a vectorized approach after reading through homework discussion forums
% y_m = eye(num_labels)(y,:);
% J = 1/m * ( trace(-y_m' * log(h)) - trace(1 - y_m' * log(1 - h)));
...
t1 = Theta1;
t1(:,1) = 0;
grad_reg1 = lambda/m * t1;
Theta1_grad = Theta1_grad + grad_reg1;


t2 = Theta2;
t2(:,1) = 0;
grad_reg2 = lambda/m * t2;
